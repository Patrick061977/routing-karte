 ...
 *
 * @package raven
 * Fixed for PHP 8.0/8.1 compatibility: replaced curly brace string access $path{0} with $path[0]
 */
class Raven_Client
{
    const VERSION = '1.9.2';
    ...
    private static function _convertPath($value)
    {
        ...
        // prefix is matched
        //
        // FIX: Changed $path{0} to $path[0] for PHP 8.0+ compatibility
        ...
        do {
            ...
            $exc_data['stacktrace'] = array(
                'frames' => Raven_Stacktrace::get_stack_info(
                    $trace, $this->trace, $vars, $this->message_limit,
                    $this->prefixes, $this->app_path, $this->excluded_app_paths,
                    $this->serializer, $this->reprSerializer
                ),
            );
    ...
    public function capture($data, $stack = null, $vars = null)
    {
        ...
        $data['tags'] = array_merge(
            $this->tags, $this->context->tags, $data['tags']);

        $data['extra'] = array_merge(
            $this->get_extra_data(), $this->context->extra, $data['extra']);
        ...
        $existing_runtime_context = isset($data['contexts']['runtime'])
        ...
        if (! empty($stack) && ! isset($data['stacktrace']) && ! isset($data['exception'])) {
            $data['stacktrace'] = array(
                'frames' => Raven_Stacktrace::get_stack_info(
                    $stack, $this->trace, $vars, $this->message_limit,
                    $this->prefixes, $this->app_path, $this->excluded_app_paths,
                    $this->serializer, $this->reprSerializer
                ),
            );
